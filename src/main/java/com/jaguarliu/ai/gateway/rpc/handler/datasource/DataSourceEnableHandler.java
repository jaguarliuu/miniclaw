...
/**
 * datasources.enable - 启用数据源
 */
@Slf4j
@Component
@RequiredArgsConstructor
public class DataSourceEnableHandler implements RpcHandler {
    ...
    public Mono<RpcResponse> handle(String connectionId, RpcRequest request) {
        return Mono.fromCallable(() -> {
            ...
            return RpcResponse.success(request.getId(), Map.of("enabled", true));
            ...
            return Mono.just(RpcResponse.error(request.getId(), "ENABLE_FAILED",
                    "Failed to enable data source: " + e.getMessage()));
        });
    }
}
